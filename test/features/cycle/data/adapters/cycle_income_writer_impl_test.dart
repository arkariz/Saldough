...
void main() {
  ...
  setUp(() {
    ...
    when(() => repository.saveCycle(any())).thenAnswer((_) async => right(unit));
  });

  group('CycleIncomeWriterImpl', () {
    test('membuat baris pemasukan baru kalau belum ada baris ber-sourceId itu', () async {
      when(() => repository.getCycle('2026-09')).thenAnswer((_) async => right(MonthlyCycle.empty('2026-09')));
      ...
      final saved = verify(() => repository.saveCycle(captureAny())).captured.single as MonthlyCycle;
    ...
    test('menimpa baris pemasukan yang sudah ada untuk sourceId yang sama', () async {
      final cycle = MonthlyCycle(
        id: '2026-09',
        incomeLines: const [IncomeLine(id: 'existing', label: 'Gaji Menul', amount: 1, sourceId: 'gaji-menul')],
        budgetLines: const [],
        investmentPlan: InvestmentPlan.empty(),
      );
      when(() => repository.getCycle('2026-09')).thenAnswer((_) async => right(cycle));
      final writer = CycleIncomeWriterImpl(cycleRepository: repository);
    ...
    test('menolak menyuntik ke siklus yang belum ada', () async {
      when(() => repository.getCycle('2026-12')).thenAnswer((_) async => right(null));
      final writer = CycleIncomeWriterImpl(cycleRepository: repository);
    ...
    test('menolak menyuntik ke siklus yang sudah ditutup', () async {
      ...
      when(() => repository.getCycle('2026-09')).thenAnswer((_) async => right(closed));
      final writer = CycleIncomeWriterImpl(cycleRepository: repository);
