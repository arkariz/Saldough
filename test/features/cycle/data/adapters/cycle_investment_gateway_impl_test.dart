...
void main() {
  ...
  setUp(() {
    ...
    gateway = CycleInvestmentGatewayImpl(
      cycleRepository: CycleRepositoryImpl(storage: storage, resolver: const UnavailableRollUpResolver()),
    );
  });

  group('CycleInvestmentGatewayImpl', () {
    test('getSnapshot membaca remainder, returnDeposit, alokasi, dan status tutup', () async {
      final repository = CycleRepositoryImpl(storage: storage, resolver: const UnavailableRollUpResolver());
      await repository.saveCycle(const MonthlyCycle(
        id: '2026-09',
        incomeLines: [IncomeLine(id: 'i1', label: 'Gaji', amount: 500000000)],
        budgetLines: [],
      ...
      final snapshot = result.getOrElse((_) => throw StateError('expected Right'));
      expect(snapshot, isNotNull);
      expect(snapshot!.remainder, 500000000);
      expect(snapshot.returnDeposit, 20000000);
      expect(snapshot.investmentBudget, 520000000);
      expect(snapshot.allocations.single, const AllocationPercentage(goalId: 'a', percentage: 50));
      expect(snapshot.isClosed, isFalse);
    });

    test('getSnapshot mengembalikan Right(null) untuk siklus yang belum ada', () async {
      final result = await gateway.getSnapshot('2099-01');
      expect(result.getOrElse((_) => throw StateError('expected Right')), isNull);
    });

    test('saveAllocationPlan menimpa rencana investasi siklus terbuka', () async {
      final repository = CycleRepositoryImpl(storage: storage, resolver: const UnavailableRollUpResolver());
      ...
      final snapshot = snapshotResult.getOrElse((_) => throw StateError('expected Right'));
    ...
    test('saveAllocationPlan menolak siklus yang sudah ditutup (ADR-0008)', () async {
      final repository = CycleRepositoryImpl(storage: storage, resolver: const UnavailableRollUpResolver());
    ...
    test('listClosedCycleSnapshots hanya mengembalikan siklus yang sudah ditutup', () async {
      final repository = CycleRepositoryImpl(storage: storage, resolver: const UnavailableRollUpResolver());
      await repository.saveCycle(MonthlyCycle.empty('2026-08').close());
      await repository.saveCycle(MonthlyCycle.empty('2026-09'));
      ...
      final snapshots = result.getOrElse((_) => throw StateError('expected Right'));
