...
void main() {
  ...
  group('RollOverCycle', () {
    test('membuat siklus bulan berikutnya dari template', () async {
        ...
        (_) async => right(
          CycleTemplate(
            ...
            defaultAllocations: const [
              Allocation(goalId: 'kyoto', percentage: 20),
            ],
          ),
        ),
      );
      ...
      final manual = cycle.budgetLines.firstWhere(
        (l) => l.kind == BudgetLineKind.manual,
      );
      expect(manual.amount, 150000000);
      expect(manual.needsReview, isTrue);

      final rollUp = cycle.budgetLines.firstWhere(
        (l) => l.kind == BudgetLineKind.rollUp,
      );
      expect(
        rollUp.amount,
        0,
    ...
    test('Desember berpindah ke Januari tahun berikutnya', () async {
      when(
        () => cycleRepository.getCycle('2027-01'),
      ).thenAnswer((_) async => right(null));
      when(() => templateRepository.getTemplate()).thenAnswer(
