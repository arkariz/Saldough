...
void main() {
  ...
  /// Membuka formulir TAMBAH ([editing] `null`), atau membuka
  /// [WalletDetailPage] lewat kartu dompet [editing] lalu tombol "Sunting"
  /// di sana untuk membuka formulir SUNTING (T-2.8 -- kartu dompet tidak
  /// lagi membuka formulir langsung).
  Future<void> openForm(WidgetTester tester, {String? editing}) async {
    if (editing == null) {
      final target = find.widgetWithText(AppButton, t.wallet.addAction);
    ...
    await tester.pumpAndSettle();
    await tester.tap(
      find.byWidgetPredicate((w) => w is AppIcon && w.iconKey == IconKey.edit),
    );
    await tester.pumpAndSettle();
  }
  ...
  group('WalletListPage', () {
    testWidgets(
      'belum ada dompet: keadaan kosong dengan ajakan menambah dompet pertama',
      (tester) async {
    ...
    testWidgets(
      'menampilkan total saldo dompet AKTIF dan tiap dompet dengan saldonya',
      (tester) async {
        tallViewport(tester);
    ...
    testWidgets(
      'saldo negatif tampil dengan tanda minus dan warna expense, bukan sebagai kesalahan (FR-WAL-003)',
      (
        tester,
      ) async {
        tallViewport(tester);
    ...
    testWidgets(
      'menambah dompet: tersimpan dengan saldo awal, tampil di daftar, dan TANPA transaksi',
      (tester) async {
        tallViewport(tester);
        await openWalletsTab(tester);
        ...
        await tester.pump();
        await tester.tap(
          find.widgetWithText(AppButton, t.wallet.saveAddAction),
        );
        await tester.pumpAndSettle();
        ...
        expect(
          transactions,
          isEmpty,
          reason: 'saldo awal bukan transaksi setoran',
        );
      },
    );

    testWidgets('menyunting nama tidak mengubah saldo; daftar ikut berubah', (
      tester,
    ) async {
      tallViewport(tester);
    ...
    testWidgets(
      'mengganti saldo awal menghitung ulang saldo tercatat dari transaksi yang ada',
      (tester) async {
        tallViewport(tester);
    ...
    testWidgets(
      'menonaktifkan dompet memindahkannya ke bagian nonaktif dan mengeluarkannya dari total',
      (
        tester,
      ) async {
        tallViewport(tester);
        ...
        expect(
          find.text('Rp1.000.000'),
          findsWidgets,
          reason: 'total kini hanya BCA',
        );
      },
    );

    testWidgets(
      'menghapus dompet kosong lewat konfirmasi: hilang, dan kembali ke keadaan kosong',
      (tester) async {
        tallViewport(tester);
        ...
        await tester.ensureVisible(
          find.widgetWithText(AppButton, t.wallet.deleteAction),
        );
    ...
    testWidgets(
      'dompet yang sudah punya transaksi TIDAK terhapus: ada pesan, dompet tetap',
      (tester) async {
        tallViewport(tester);
        await seed('a', 'BCA', current: 10000000);
        await transactionRepository.saveTransaction(
    ...
    testWidgets(
      'membuka tab Dompet lagi menyegarkan saldo yang berubah di tempat lain',
      (tester) async {
        tallViewport(tester);
        await seed('a', 'BCA', current: 100000000);
        await openWalletsTab(tester);
    ...
    testWidgets(
      'layar 360px + teks 2x + nama panjang + saldo besar: tidak overflow, nama tidak dielipsis',
      (
        tester,
      ) async {
    ...
    testWidgets(
      'halaman memakai bahasa pencatatan: tidak ada kosakata yang menyiratkan memindahkan uang',
      (
        tester,
      ) async {
        tallViewport(tester);
        ...
        final texts = tester
            ...
            .map(
              (w) => (w.data ?? w.textSpan?.toPlainText() ?? '').toLowerCase(),
            );
        for (final text in texts) {
